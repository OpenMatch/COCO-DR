...
class CondenserForPretraining(nn.Module):
    ...
    def forward(self, model_input, labels, groups: Tensor = None, **kwargs):
...
class CoCondenserForPretraining(CondenserForPretraining):
    ...
    def forward(self, model_input, labels, groups: Tensor = None, grad_cache: Tensor = None, chunk_offset: int = None):
        attention_mask = self.lm.get_extended_attention_mask(
            model_input['attention_mask'],
            model_input['attention_mask'].shape,
            model_input['attention_mask'].device
    ...
    def compute_contrastive_loss(self, co_cls_hiddens):
        ...
        batch_co_loss = F.cross_entropy(similarities, self.co_target, reduction = 'none') * self._world_size()
        return batch_co_loss
