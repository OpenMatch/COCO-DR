...
def passage_dist_eval_ance(args, model, tokenizer, dataset = 'marco'):
    base_path = args.beir_data_dir
    passage_path = os.path.join(base_path, f"collections.{dataset}.tsv")
    queries_path = os.path.join(base_path, f"queries.dev.{dataset}.tsv")
    def fn(line, i):
        return dual_process_fn(line, i, tokenizer, args)
    mrr_ref_path = os.path.join(base_path, f"qrels.dev.{dataset}.tsv")
    ref_dict = load_reference(mrr_ref_path)
    full_ranking_mrr = combined_dist_eval_ance(
        args, model, queries_path, passage_path, fn, fn, ref_dict)
    return full_ranking_mrr 
    

def combined_dist_eval_ance(args, model, queries_path, passage_path, query_fn, psg_fn, ref_dict):
    # get query/psg embeddings here
    eval_batch_size = args.per_gpu_eval_batch_size * max(1, args.n_gpu) // 2
    query_embs, query_ids = embedding_inference(
        args, queries_path, model, query_fn, eval_batch_size, 0, True)
    ...
    print("query", query_embs.shape)
    psg_embs, psg_ids = embedding_inference(
        args, passage_path, model, psg_fn, eval_batch_size, 0, False)
    print("passage", psg_embs.shape)
...
def passage_dist_eval(args, model, tokenizer):
    ...
    # fast evaluation with small documents
    passage_path = os.path.join(base_path, "collection.title.tsv")
...
def compute_ndcg(D, I, qids, ref_dict):
    knn_pkl = {"D": D, "I": I}
    all_knn_list = all_gather(knn_pkl)
    mrr = 0.0
    if is_first_worker():
