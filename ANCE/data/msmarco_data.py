...
def write_query_rel(args, pid2offset, query_file, positive_id_file, out_query_file, out_id_file):
    ...
    embedding_cache = EmbeddingCache(out_query_path, group = False)
    print("First line")
    ...
    with gzip.open(query_positive_id_path, 'rt', encoding='utf8') if positive_id_file[-2:] == "gz" else open(query_positive_id_path, 'r', encoding='utf8') as f, \
        ...
        for [topicid, _, docid, rel] in tsvreader:
            ...
            if args.data_type == 0:
                ...
                docid = int(docid)
            # print(query_positive_id_path, topicid, docid, rel)
...
def preprocess(args):
    ...
    with open(out_passage_path + "_meta", 'w') as f:
        json.dump(meta, f)
    embedding_cache = EmbeddingCache(out_passage_path, group = False)
    print("First line")
    with embedding_cache as emb:
        print(emb[0])
    ...
    if args.data_type == 0:
        ...
        #         args,
        #         pid2offset,
        #         "queries.train.%s.tsv"%(args.name),
        #         "qrels.train.%s.tsv"%(args.name),
        #         "train-query",
        #         "train-qrel.tsv")
        # except:
        write_query_rel(
        ...
        #         args,
        #         pid2offset,
        #         "queries.dev.small.%s.tsv"%(args.name),
        #         "qrels.dev.small.%s.tsv"%(args.name),
        #         "dev-query",
        #         "dev-qrel.tsv")
        # except:
        write_query_rel(
...
def PassagePreprocessingFn(args, line, tokenizer):
    if args.data_type == 0:
        ...
        if 'condenser' in args.model_type:
            p_text = p_text.lower()
...
def get_arguments():
    ...
    #     default=0,
    #     type=int,
    #     help="0 for not add title, 1 for ading title",
    # )
    # parser.add_argument(
    #     "--name",
    #     default='',
    #     type=str,
    #     help="Name for question generation file",
    # )
