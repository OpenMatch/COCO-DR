...
def StreamInferenceDoc(args, model, fn, prefix, f, is_query_inference=True):
    ...
    logger.info("Init inference dataset")
    # fn: the dataset Getprocessfn(vals, i) : vals: [passage_len, passage] (in f)
    # f: emb see line 278 of utils.py （multi elements)
    inference_dataloader = DataLoader(
        inference_dataset,
        batch_size=inference_batch_size)
    if args.local_rank != -1:
        dist.barrier()  # directory created
    logger.info("Begin inferencing embeddings")
...
def ann_data_gen(args):
    ...
    ann_no, ann_path, ndcg_json, _ = get_latest_ann_data(args.output_dir)
    print("Finish ANN Data")
    output_num = ann_no + 1
