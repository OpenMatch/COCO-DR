...
def PassagePreprocessingFn(args, line, tokenizer):
    if args.data_type == 0:
        ...
        if args.title == 1:
            ...
            p_id = int(line.get("_id"))
            if line.get("title") != "":
                p_text = line.get("title") + ' ' + line.get("text")
            else:
                p_text = line.get("text")
        else:
            # print("NO TITLE!")
            line_arr = line.split('\t')
            p_id = int(line_arr[0])
...
def QueryPreprocessingFn(args, line, tokenizer):
    ...
    if args.group == 1:
        ...
        return q_id.to_bytes(8,'big') + q_group.to_bytes(4,'big') + passage_len.to_bytes(4,'big') + np.array(input_id_b,np.int32).tobytes()


def GetProcessingFn(args, query=False, beir = False):
    def fn(vals, i):
        if args.group == 0 or beir == True:
            passage_len, passage = vals
        else:
            group_id, passage_len, passage = vals
        max_len = args.max_query_length if query else args.max_seq_length
        # if args.max_seq_length < 512:
        #     print("Warning Max Seq < 512!")
        ...
        if args.group == 0 or beir == True:
            ...
            passage_collection = [(i, passage, attention_mask, token_type_ids,  group_id)]
            ...
            all_token_type_ids_a = torch.tensor(
                [f[3] for f in passage_collection], dtype=torch.uint8)
            all_group_id_a =  torch.tensor(
                [f[-1] for f in passage_collection], dtype=torch.long)
    ...
    def fn(line, i):
        ...
        for neg_pid in neg_pids:
            ...
            if args.group == 0:
                ...
                yield (query_data[0], query_data[1], query_data[2], pos_data[0], pos_data[1], pos_data[2], pos_label, query_data[-1])
                yield (query_data[0], query_data[1], query_data[2], neg_data[0], neg_data[1], neg_data[2], neg_label, query_data[-1])          
    return fn
    ...
    def fn(line, i):
        ...
        for neg_pid in neg_pids:
            ...
            if args.group == 0:
                ...
                yield (query_data[0], query_data[1], query_data[2], pos_data[0], pos_data[1], pos_data[2],
                       neg_data[0], neg_data[1], neg_data[2], query_data[-1], query_data[3])
...
def get_arguments():
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="0 for not add title, 1 for adding title",
    )
    parser.add_argument(
        "--group",
        default=0,
        type=int,
        help="0 for not grouping, 1 for adding grouping",
    )
