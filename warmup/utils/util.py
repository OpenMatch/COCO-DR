...
def get_latest_ann_data(ann_data_path, result_dir = None, dataset = ''):
    if not result_dir:
    ...
    if len(data_no_list) > 0:
        ...
        if dataset != '':
            ...
            for d in datasets:
                try:
                    with open(os.path.join(result_dir,  '%s/'%(d) + ANN_PREFIX +str(data_no)), 'r') as f:
                        ndcg = json.load(f)
                        ndcg_beir[d] = ndcg['ndcg']
                except:
                    ndcg_beir[d] = 0 # the file is not find
            if result_dir == ann_data_path:
                return data_no, os.path.join(
                    ann_data_path, "ann_training_data_" + str(data_no)), ndcg_json, ndcg_beir
            else:
                return data_no, os.path.join(
                    ann_data_path, "msmarco/ann_training_data_" + str(data_no)), ndcg_json, ndcg_beir
        else:
            if result_dir == ann_data_path:
                return data_no, os.path.join(
...
class EmbeddingCache:
        ...
        with open(base_path + '_meta', 'r') as f:
            ...
            if self.group:
                self.record_size = int(
                    meta['embedding_size']) * self.dtype.itemsize + 4 + 4           
            else:
                self.record_size = int(
    ...
    def read_single_record_with_group(self):
        record_bytes = self.f.read(self.record_size)
        group_id =  int.from_bytes(record_bytes[:4], 'big')
        passage_len = int.from_bytes(record_bytes[4:8], 'big')
        passage = np.frombuffer(record_bytes[8:], dtype=self.dtype)
        # print(group_id, passage_len, passage )
        return group_id, passage_len, passage    
    ...
    def __getitem__(self, key):
        ...
        if self.group:
            return self.read_single_record_with_group()
        else:
