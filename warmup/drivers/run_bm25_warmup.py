...
import transformers
from utils.eval_mrr import passage_dist_eval, passage_dist_eval_ance
...
from utils.util import getattr_recursive, set_seed, is_first_worker, StreamingDataset, get_checkpoint_no
import copy
from model.models import mt_update
from model.loss import SymKlCriterion, NsSymKlCriterion
import math
try:
...
def train(args, model, tokenizer, f, train_fn, global_step = 0):
    """ Train the model """
    tb_writer = None
    if is_first_worker():
    ...
    if is_first_worker():
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                ...
                total_trainable_params += param.shape[0] if len(param.shape) == 1 else param.shape[0] * param.shape[1]
    ...
    if os.path.exists(args.model_name_or_path) and not args.from_scratch:
    ...
    print(f"Begin training! World size: {torch.distributed.get_world_size()}")
    ...
    for m_epoch in train_iterator:
        ...
        for step, batch in tqdm(enumerate(epoch_iterator),desc="Iteration",disable=args.local_rank not in [-1,0]):
            ...
            if hasattr(model, "module"):
                correct = model.module.correct
                total = model.module.total
            else:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0 or global_step == 1:
                    logs = {}
                    if args.evaluate_during_training and global_step % (args.logging_steps_per_eval*args.logging_steps) == 0 or global_step == 1:
                        model.eval()
                    ...
                    logs["train_acc"] = (correct)/(total+1e-10)
                    correct= 0 
                    total = 0
                    if hasattr(model, "module"):
                        model.module.set_zero()
                    else:
                        model.set_zero()
...
def get_arguments():
    ...
    parser.add_argument(
        ...
        default=1,
        type=float,
        help="Whether train from scratch or load previous models.",
    )
...
def set_env(args):
    ...
    # print("___________")
    # print(os.environ, dict(os.environ)["RANK"], dict(os.environ)["MASTER_ADDR"], dict(os.environ)["MASTER_PORT"], )
    args.rank =  int(dict(os.environ)["RANK"])
    args.server_ip = dict(os.environ)["MASTER_ADDR"]
    args.server_port = dict(os.environ)["MASTER_PORT"]
    # print(args.server_ip, args.server_port, args.local_rank)
    # print("___________")
    # exit()
...
def get_latest_checkpoint(args):
    if not os.path.exists(args.output_dir):
        return args.model_name_or_path, 0
    subdirectories = list(next(os.walk(args.output_dir))[1])
    
    def valid_checkpoint(checkpoint):
        chk_path = os.path.join(args.output_dir, checkpoint)
...
def main():
    ...
    if latest_step_num > 0:
        args.model_name_or_path = next_checkpoint
        global_step = latest_step_num
    else:
        global_step = 0
    # else:
    print(f"Loading models from {args.model_name_or_path}, train from step {global_step}")
    ...
    if args.do_train:
        ...
        with open(args.train_data_dir, encoding="utf-8-sig") as f:
            train_batch_size = args.per_gpu_train_batch_size * \
                max(1, args.n_gpu)
            global_step, tr_loss = train(
                args, model, tokenizer, f, train_fn, global_step)
...
if __name__ == "__main__":
    ...
    logger.info("___________Begin training________")
    main()
